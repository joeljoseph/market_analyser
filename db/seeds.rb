...
currencies.each do |key,value|
  Currency.create(code: key, name: value )
end
...
data.each do |currency_data|
	p ary = currency_data.split("\n\t\t\t\t\t")
    c =	Currency.where(code: ary[1]).first
	#["", "SHP", "£", "654", "Saint Helena Pound", "", "\t", "\t\tAscension Island", "\t\tSaint Helena", "\t\tTristan da Cunha", "\t", "\n\t\t\t\t"]
	c.update_attributes(symbol: ary[2], digital_code: ary[3], name: ary[4], country: ary[5..-1].join(',') ) if c.present?
end